...
class MatchGame extends Component {
  ...
  imageMatch = url => {
    ...
    if (randomImageUrl === url) {
      this.setState(prevState => ({
        score: prevState.score + 1,
        randomImageUrl: this.randomImageGenerator(),
      }))
    } else {
      this.setState({isGameRunning: false})
      clearInterval(this.timerId)
  ...
  randomImageGenerator = () => {
    ...
    return randomImageObject ? randomImageObject.imageUrl : ''
  }

  getActiveTabApps = () => {
    const {activeTab} = this.state
    const filteredImages = imagesList.filter(
      eachSearchedImage => eachSearchedImage.category === activeTab,
    )
  ...
  renderGame() {
    const {activeTab, randomImageUrl} = this.state
    const filteredResults = this.getActiveTabApps()
    return (
      <div className="game-container">
        <img src={randomImageUrl} alt="match" className="view-image" />
    ...
    return (
      <div className="result-container">
        <img
          ...
          className="trophy"
        />
        <h1 style={{color: '#ffffff'}}>YOUR SCORE</h1>
        <h1 style={{color: '#ffffff'}}>{score}</h1>
  ...
  render() {
    const {time, score, isGameRunning} = this.state
    console.log(score)
    ...
    return (
      <div className="main-container">
        ...
        <div className="game-container">
          {isGameRunning ? this.renderGame() : this.renderResult()}
        </div>
      </div>
    )
  }
